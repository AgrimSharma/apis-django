...
from .serializers import SymptomsRecordSerializer, SymptomsUserSerializer, SymptomsDefSerializer, SymptomsRecordUser, \
    SymptomsUserList, SymptomsByName, DefSerializer
...
class SymptomsListAPI(viewsets.ModelViewSet):
    """
    <h4>SymptomsList API endpoint that allows symptoms to be viewed or edited.</h4>
    """
...
class SymptomsAPI(viewsets.ModelViewSet):
    """
    <h4>SymptomsAPI endpoint that allows symptoms to be viewed or edited.<h4>
    """
...
class SymptomsRecordsAPI(viewsets.ModelViewSet):
    """
    <h4>SymptomsRecord API endpoint that allows symptoms records to be viewed or edited.</h4>
    """
...
class SymptomsRecordsUserAPI(GenericAPIView):
    ...
    def post(self, request):
        ...
        <h4>List Symptoms Records By user ID</h4>
        """
        user=SymptomsRecord.objects.filter(userID__id=request.data['userID'], status=True)
        result=[]
        for x in user:
...
class SymptomsListUserAPI(GenericAPIView):
    ...
    def post(self, request):
        ...
        <h4>List Symptoms by user ID</h4>
        """
        symptoms=SymptomsUser.objects.filter(userID__id=request.data['userID'])
        response=[]
        for s in symptoms:
            user=UserSerializerReport(s.userID).data
            response.append(
                dict(user=user, symptoms_user=SymptomsUserSerializer(s).data, symptoms_user_id=s.id))
